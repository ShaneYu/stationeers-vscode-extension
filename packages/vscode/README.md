...
StationeersLua is optional. Toolkit includes a lightweight fallback Lua library
so Lua editing works without it. If both extensions are installed, Toolkit uses
the current StationeersLua library for the broadest Lua completions, hovers, and
diagnostics, while adding only Toolkit-specific metadata. This lets the Lua
knowledge update with StationeersLua without requiring a Toolkit release.
...
Install both extensions if you work extensively with Lua chips. Do not use both
extensions' live network, chip editor, or source push/pull tools against the
same game session at the same time. Choose one live integration owner; Toolkit
is recommended when you also need IC10 handling, simulations, scenario testing,
...
- Visual `*.icsim` environments for devices, labeller names, numbered
...
Every `*.icsim` file in each workspace folder is indexed against the
program path and stable program/device ID it declares; When an IC10 editor is
...
Put repeatable cases in `*.ictest` beside a reusable simulation
environment. Obsolete scenario-test filenames are rejected and reported with the required rename. Test Explorer discovers file, case, and parameter levels and can
...
- **IC10: Create Simulation Environment** — creates a source-controlled
  `*.icsim` environment and opens its visual editor.
...
- **IC10: Create Scenario Test** — creates a source-controlled
  `*.ictest` fixture and opens its guarded visual editor.
