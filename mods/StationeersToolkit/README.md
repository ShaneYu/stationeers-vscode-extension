...
```powershell
dotnet restore .\mods\StationeersToolkit\StationeersToolkit.sln
dotnet build .\mods\StationeersToolkit\StationeersToolkit.sln --configuration Release --no-restore
dotnet run --project .\mods\StationeersToolkit.Tests\StationeersToolkit.Tests.csproj
```
...
`StationeersToolkit.dll`, `StationeersToolkit.Core.dll`,
and the dependency manifest into:

```text
%USERPROFILE%\Documents\My Games\Stationeers\mods\StationeersToolkit
```
...
game update loop. `Relay:AllowRemoteWrites` defaults to `true`; disable it when
authenticated remote writes are not wanted.
